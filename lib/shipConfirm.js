...
ShipConfirm.prototype.makeRequest = function(options) {
 ...
 function buildRequestData(data) {
    ...
    response += "<ShipmentConfirmRequest>";
      ...
      if(data.shipment.ReturnService) {
        response += "<ReturnService>";
          response += "<Code>";
          response += data.shipment.ReturnService.Code;
          response += "</Code>";
          response += "<Description>";
          response += data.shipment.ReturnService.Description;
          response += "</Description>";
				response += "</ReturnService>";
			}
      ...
      if(!data.shipment.payment && !data.shipment.itemPayment)  return { success: false, error: 'Missing Shipment Payment' };
      ...
      if(data.shipment.payment) {
        ...
        response += "</PaymentInformation>";
			...
			if(data.shipment.itemPayment) {
        var payment = data.shipment.itemPayment;
        
        response += "<ItemizedPaymentInformation>";
        ...
        response += "</ItemizedPaymentInformation>";
			...
			if (data.shipment.isDropOff) {
				response += "<DropoffAtUPSFacilityIndicator>";
				response += "</DropoffAtUPSFacilityIndicator>";
			}
		...
		if(!data.LabelSpecification)  return { success: false, error: 'Missing Label Specification' };
		response += "<LabelSpecification>";
			
			response += "<LabelPrintMethod>";
				response += "<Code>";
				response += data.LabelSpecification.LabelPrintMethodCode || 'GIF';
				response += "</Code>";
			response += "</LabelPrintMethod>";

      if(data.LabelSpecification.LabelStockSize) {
        response += "<LabelStockSize>";
          response += "<Height>";
            response += data.LabelSpecification.LabelStockSize.Height;
          response += "</Height>";
          response += "<Width>";
            response += data.LabelSpecification.LabelStockSize.Width;
          response += "</Width>";
		  	response += "</LabelStockSize>";
      }
			...
			response += "<LabelImageFormat>";
				response += "<Code>";
				response += data.LabelSpecification.LabelImageFormatCode || 'GIF';
				response += "</Code>";
			response += "</LabelImageFormat>";
...
var buildPackageInternals = function(val) {
    ...
    response += "<UnitOfMeasurement>";
      response += "<Code>";
        response += val.weightMeasurement || "KGS"
      response += "</Code>";
    response += "</UnitOfMeasurement>";
	response += "</PackageWeight>";
  ...
  if (val.referenceNumber) {
    ...
    response += "<BarCodeIndicator>" + val.referenceNumber.barCodeIndicator + "</BarCodeIndicator>"; // value inside is ignored
    response += "<Code>" + val.referenceNumber.code + "</Code>";
    response += "<Value>" + val.referenceNumber.value + "</Value>";
    response += "</ReferenceNumber>";
  }
